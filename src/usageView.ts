...
import { getChatImportStats } from './chatImport';
import {
  billingCycleSummaries,
  billingPeriodTotals,
  clearUsage,
  ...
  autoModelTotals,
  explicitModelTotals,
  windowAutoModelTotals,
  windowExplicitModelTotals,
  DayBucket,
  Totals,
  modelMetrics,
  vendorMetrics,
...
export function showUsageView(context: vscode.ExtensionContext): void {
  if (current) {
    current.reveal();
    return;
  }
...
export function refreshUsageView(): void {
  if (current) {
    current.webview.html = usageHtml(current.webview);
  }
}
...
function usageTrendChart(series: DayBucket[]): string {
  const W = 560;
  const H = 230;
  const padL = 46;
  const padB = 28;
  ...
  const py = (value: number) => padT + chartH - (value / max) * chartH;
  ...
  const actualDots = series
    .map((s, i) => {
      ...
      const y = py(s.cost);
      return (
        `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="2.5" class="line-dot actual-dot">` +
  ...
  const gridY = [0, 0.5, 1]
    .map((f) => {
      ...
      const val = f * max;
      return (
        `<line x1="${padL}" y1="${y.toFixed(1)}" x2="${(W - padR).toFixed(1)}" y2="${y.toFixed(1)}" class="grid"/>` +
  ...
  return (
    `<svg viewBox="0 0 ${W} ${H}" width="100%" role="img" aria-label="Daily AI cost trend with actual and forecast lines">` +
    `${gridY}` +
    `${legend}` +
...
function windowsHtml(): string {
  const cardSplit = (label: string, auto: Totals, explicit: Totals) => {
    return (
      `<div class="cardsplit">` +
  ...
  return (
    `<div class="cards">` +
    cardSplit('Last 5 hours', windowAutoModelTotals(5 * HOUR), windowExplicitModelTotals(5 * HOUR)) +
    cardSplit('Last 7 days', windowAutoModelTotals(7 * 24 * HOUR), windowExplicitModelTotals(7 * 24 * HOUR)) +
    cardSplit('Current billing period', autoModelTotals(billingPeriodRecords), explicitModelTotals(billingPeriodRecords)) +
    cardSplit('All time', autoModelTotals(), explicitModelTotals()) +
    `</div>`
  );
}

function recordsInCurrentBillingPeriod() {
  const range = currentBillingPeriodRange();
  return records().filter((record) => record.ts >= range.start && record.ts < range.endExclusive);
}

function totalsTable(t: Totals): string {
...
function monthlyBillingSummaryTable(): string {
  const rows = billingCycleSummaries(24);
  if (!rows.length) {
...
function modelMetricsTable(): string {
...
function vendorMetricsTable(): string {
