...
exports.initChatImport = initChatImport;
exports.disposeChatImport = disposeChatImport;
exports.clearChatImportState = clearChatImportState;
exports.showChatImportDiagnosticsOutput = showChatImportDiagnosticsOutput;
exports.syncChatUsage = syncChatUsage;
...
function diagnosticsEnabled() {
    return config_1.Config.diagnosticsEnabled();
}
...
function diagnosticLog(message, context, syncId) {
...
function initChatImport(context) {
    importedIdsStore = context.globalState;
    lastImportStats = importedIdsStore.get(IMPORT_STATS_KEY, lastImportStats);
    void syncChatUsage();
...
function disposeChatImport() {
    if (syncTimer) {
        clearInterval(syncTimer);
        syncTimer = undefined;
    }
}
async function clearChatImportState() {
    await importedIdsStore?.update(IMPORTED_TURN_IDS_KEY, []);
}
function showChatImportDiagnosticsOutput() {
    getDiagnosticsChannel().show(true);
}
async function syncChatUsage() {
    if (activeSync) {
...
function getChatImportStats() {
    return { ...lastImportStats };
}
async function importDebugViewText(raw) {
...
async function doSyncChatUsage(syncId) {
    if (!importedIdsStore) {
    ...
    const importedIds = new Set(importedIdsStore.get(IMPORTED_TURN_IDS_KEY, []));
    ...
    for (const turn of turns) {
        ...
        const hasExplicitTranscriptTokens = (typeof turn.userTokens === 'number' && Number.isFinite(turn.userTokens) && turn.userTokens >= 0) ||
...
function consumeBestDebugUsage(turn, modelName, usages) {
    if (!usages.length) {
        return undefined;
    }
    ...
    for (let i = 0; i < usages.length; i += 1) {
        ...
        const candidateHasCredits = typeof candidate.aiCredits === 'number' && Number.isFinite(candidate.aiCredits) && candidate.aiCredits > 0;
...
function extractUsageObject(parsed) {
    const candidates = [
        parsed?.usage,
        parsed?.data?.usage,
        parsed?.attrs?.usage,
...
function extractTokenBreakdown(usage) {
    ...
    const cachedPromptTokens = typeof usage?.prompt_tokens_details?.cached_tokens === 'number' ? usage.prompt_tokens_details.cached_tokens : 0;
 ...
 */
function parseCreditsFromChatSessionDetails(details) {
 ...
 */
function parseModelFromCreditDetails(details) {
    if (!details) {
        return undefined;
    }
...
async function parseTranscriptTurns(filePath) {
    const text = await safeReadText(filePath);
    if (!text) {
        return [];
    }
...
function extractDebugUsageTurnsFromText(content, ts, baseId) {
    if (!content) {
        return [];
    }
    const turns = [];
...
async function readCopilotRequestLogEntriesFromAppRoot(appRoot) {
    ...
    for (const logRoot of logRoots) {
        ...
        for (const entry of windowEntries) {
            ...
            const logFile = path.join(windowsDir, entry.name, 'exthost', 'GitHub.copilot-chat', 'GitHub Copilot Chat.log');
...
function parseRequestLogEntries(text) {
    const entries = [];
 ...
 */
function parseCopilotMdUsageEntry(content, ccreqId, ts) {
    let model;
    let usage;
    for (const line of content.split(/\r?\n/)) {
        if (!model) {
...
function parseDebugViewUsageEntries(text, source) {
    const entries = [];
    let pendingModel;
    for (const line of text.split(/\r?\n/)) {
...
function matchRequestModel(turn, requests) {
    let best;
    let bestDistance = Number.POSITIVE_INFINITY;
...
async function safeSelectChatModels() {
    try {
...
function findModelForName(models, modelName) {
...
function normaliseModelNeedle(value) {
...
//# sourceMappingURL=chatImport.js.map